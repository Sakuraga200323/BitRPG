...
* **＊幽人の庭師〆魂魄妖夢**＊#4548
# [主要コマンドの解説](#three)
省略形があるコマンドは"/"で区切ってあります。[ ]があるコマンドは、その中から一つだけ選んで記述してください。または書いてある内容のものを書き込んでください。※付きコマンドは現在使用不可です。  
|  コマンド  |  説明  |
| ---- | ---- |
|  **\^\^start**  |  冒険者登録をします。  |
...
|  **※\^\^item/i [アイテム名]**  |  指定アイテムを使用。アイテム：冒険者カード以外停止中。  |
|  **^^point [str, def, agi] [強化量]**  |  STPを消費してステータスを強化。  |
|  **※\^\^gentotsu**  |  魔石を消費してレベル上限を解放。停止中。  |
# [各システムの解説](#four)
<details>
<summary>
<strong>戦闘システム</strong>
</summary>
&emsp;BitRPGを導入すると、各チャンネルに共通で初期1Lvのモンスターが出現します。一体倒すごとに1Lvずつ上昇していきます。Normalの敵は敵ごとにステータスの特徴があり、また特定の条件下で、通常よりも強かったり、経験値量が多いモンスターが出現します。※現在Normal以外の強敵のステータスの強化をカットし、獲得経験値量とお金の量の補正だけ行っています。稀にUltraRareが出現しますが、攻撃が通らないので^^resetで敵を変更して引き続きお楽しみください。
&emsp;
<details>
<summary>
...
&emsp;BitRPGではターン制を戦闘システムに採用しています。各ターンには先手後手の概念が存在しています。一般的にはモンスターとプレイヤーのAGIを比較し、優っているほうが先手です。<br>&emsp;攻撃をした時、モンスターとプレイヤーは共に一定の確率で通常よりも高いダメージ値を叩き出します。これはわかりやすくいうとクリティカルです。ダメージの上昇量には<strong>強・超・極</strong>と段階があります。
&emsp;
<details>
<summary>
