...
async def on_message(message):
    ...
    if m_ctt.startswith("^^") and not m_author.id in check_macro.macro_checking and not m_author.bot:
        ...
        if m_ctt == '^^start':
            ...
            cmd2 = (f"INSERT INTO weapon_tb (id,player_id,name,emoji,rank) VALUES ({weapon_id},{m_author.id},'{weapon_name}','{list(box.shop_weapons[weapon_name][0]}',2);")
            try:
                pg.execute(cmd)
                pg2.execute(cmd2)
