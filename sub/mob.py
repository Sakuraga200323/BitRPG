...
def appear(m_data):
    ...
    pg.execute(f"update mob_tb set name = '{name}',lv = {lv},max_hp = {100*(lv+1)*num},now_hp = {100*(lv+1)*num},str = {10*(lv+1)*num},def = {10*(lv+1)*num},agi = {10*(lv+1)*num*agi_num},img_url = '{url}' where id = {m_data['id']};")
    embed = discord.Embed(
