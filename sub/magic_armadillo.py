...
async def open_magic(user,ch):
    ...
    magic_em.add_field(name="`1.`DrumFang",value=f">>> 必要熟練度.**0**\n消費MP.**30**\nStrength**{m1_num:.2f}(Limit150)**%の攻撃魔法 **50**%で敵に**5**ターンNerf付与 ",inline=False)
    ...
    magic_em.add_field(name="`4.`InversionemAegis",value=f"{'>>> ' if m_lv>=1000 else ''}必要熟練度.**2000**\n消費MP.**300**\n現在のDefenceを**{m4_num:.2f}(Limit500)%**に強化 後手確定 敵の攻撃で死ななかった場合 自分が受けているダメージ量の攻撃をする HPが減っていない場合通常攻撃",inline=False)
